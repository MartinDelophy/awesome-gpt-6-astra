<div align="center">

![Awesome GPT-6 Astra](assets/banner.svg)

# Awesome GPT-6 Astra

[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat-square)](CONTRIBUTING.md) [![LINUX DO](https://img.shields.io/badge/LINUX%20DO-Community-f0b73f?style=flat-square)](https://linux.do/) [![Cases: 49](https://img.shields.io/badge/Cases-49-58a6ff?style=flat-square)](https://astragames.aigccreative.com/)

**Eine Sammlung interessanter Spiele, die mit GPT-6 Astra entwickelt wurden.**

Spielerische Ideen, direkt erlebbare Spiele und Entwicklungsgeschichten, die andere zum Gestalten anregen.

[English](README.md) · [简体中文](README.zh-CN.md) · [日本語](README.ja.md) · [한국어](README.ko.md)<br>
[Français](README.fr.md) · **Deutsch** · [Español](README.es.md) · [Português (Brasil)](README.pt-BR.md)<br>
[Русский](README.ru.md) · [العربية](README.ar.md) · [हिन्दी](README.hi.md) · [Bahasa Indonesia](README.id.md)

[Spiel vorschlagen](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/new?template=submit-game.yml) · [Mitmachen](CONTRIBUTING.md)

</div>

Diese Seite übersetzt die [englische README](README.md). Aktuelle Änderungen lassen sich dort nachprüfen; Korrekturen an der Übersetzung sind willkommen.

## Hier anfangen

Entdecke **49 Spiele und interaktive Projekte**: Gebietsstrategie zur Zeit der Drei Reiche, Holz-Steckpuzzles und Schiebepuzzles lösen, weiche Früchte verschmelzen, mit einer Taste fliegen, auf einem fliegenden Teppich kämpfen, eine Insel mit einem Stromnetz verteidigen, in der Wildnis überleben, unter Wasser fischen, ein Sushi-Restaurant betreiben und eine Insel bewirtschaften, Kartrennen auf Bay Circuit, eine Küstenradtour mit einem Pelikan, Tischspielzeug als 3D-Spiele und Orbital Garden. Ein Klick auf einen Titel öffnet das Spiel direkt im Browser.

Katalog aktualisiert: **2026-09-12**. Angaben zur Modellnutzung beruhen auf Aussagen der Ersteller oder Einreichenden; unbestätigte Details sind im jeweiligen Eintrag markiert. Das Datum bezeichnet die Katalogpflege, keinen erneuten Spieltest aller Spiele.

- **Du möchtest spielen?** Stöbere in den Genres unten.
- **Du hast ein Spiel entwickelt?** [Reiche dein Projekt ein](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/new?template=submit-game.yml) und füge einen direkten Link zum Browserspiel, einen Screenshot aus dem Spiel und eine Beschreibung deiner Nutzung von GPT-6 Astra hinzu.
- **Du hast ein tolles Projekt entdeckt?** Empfiehl öffentlich zugängliche Werke anderer und nenne ihre Urheber.

Diese Liste wird von der Community gepflegt und ist nicht mit OpenAI verbunden. Ein Eintrag lädt zum Erkunden ein und ist weder ein Leistungstest noch eine offizielle Empfehlung.

## Spiele

- [Action und Arcade](#action-arcade)
- [Rätsel und Denkspiele](#puzzles)
- [Strategie und Simulation](#strategy-simulation)
- [Rollenspiele und Abenteuer](#rpg-adventures)
- [Plattform- und Rennspiele](#platformers-racing)
- [Experimentelles und Mehrspieler](#experimental-multiplayer)

<a id="action-arcade"></a>

### Action und Arcade

Shooter, Kampf-, Überlebens- und Rhythmusspiele sowie alles, was zu einer weiteren Runde einlädt.

- **[Mosswing](https://mosswing-quiet-flight.jack-514.chatgpt.site/)** — Ein 3D-Flugspiel mit nur einer Taste: Mit Flügelschlägen durch Lücken fliegen und Punkte sammeln.
  - Entwickler: [Ayi1337](https://github.com/Ayi1337)
  - Plattform: Browser, für Mobilgeräte entwickelt.
  - GPT-6 Astra: [One-Shot-Tests und ursprüngliche Prompts des Entwicklers](https://github.com/Ayi1337/gpt6-astra-one-shot-games/blob/main/README.md).
  - Materialien: [Quellcode](https://github.com/Ayi1337/gpt6-astra-one-shot-games/tree/main/mosswing/src) · [Eigenständige HTML-Datei](https://github.com/Ayi1337/gpt6-astra-one-shot-games/blob/main/mosswing/mosswing.html)
  - Vorschau: ![Mosswing-Startbildschirm mit der fliegenden Spielfigur und den Lücken zwischen Steinsäulen.](assets/screenshots/mosswing/gameplay.jpg)

- **[Stadium Elite — El Clásico](https://stadium-elite.mindblown.ai/)** — Spiele ein Fußballspiel mit elf Spielern pro Team zwischen Barcelona und Real Madrid im 3D-Stadion, mit Pässen, Schüssen und Spielerwechseln.
  - Entwickler: [Mindblown / @mind](https://mindblown.ai/@mind)
  - Plattform: Desktop-Browser; Tastatur und Maus, keine Anmeldung erforderlich.
  - GPT-6 Astra: [Prüfnotizen](assets/screenshots/stadium-elite/SOURCE.md) — Der bereitgestellte Mindblown-Beitrag nennt Three.js und GPT-6 Astra.
  - Vorschau: ![Stadium Elite — El Clásico: Spiele ein Fußballspiel mit elf Spielern pro Team zwischen Barcelona und Real Madrid im 3D-Stadion, mit Pässen, Schüssen und Spielerwechseln.](assets/screenshots/stadium-elite/gameplay.png)

- **[Magic Carpet Wizard — A Thousand Skies](https://threapchills.github.io/MagicCarpetWizard/)** — Fliege auf einem Teppich durch eine kugelförmige Welt, durchquere Ringe, wirke Zauber und bekämpfe Gegner und Bosse.
  - Entwickler: [threapchills](https://github.com/threapchills)
  - Plattform: Desktop-Browser mit Maus und Tastatur; benötigt WebGL 2.
  - GPT-6 Astra: Der Entwickler gibt im [About-Bereich des Repositorys](https://github.com/threapchills/MagicCarpetWizard) an, das Spiel mit GPT-6 Astra erstellt zu haben.
  - Materialien: [Quellcode und Startanleitung](https://github.com/threapchills/MagicCarpetWizard) · Technik: Three.js, Vite.
  - Vorschau: ![Magic-Carpet-Wizard-Startbildschirm mit Teppichreiter, Stadt und leuchtendem Flugring.](assets/screenshots/magic-carpet/gameplay.jpg)

- **[THUNDERFALL / 雷霆战机 · 天穹远征](https://thunderfall.vercel.app/)** — Ein Bullet-Hell-Shooter mit drei Jägern, fünf Sektoren, mehrphasigen Bossen und automatischem Feuer.
  - Entwickler: [jackroc](https://github.com/jackroc)
  - GPT-6 Astra: [README](works/thunderfall/README.md#创作与许可) — Laut Ersteller mit GPT-6 Astra ultra entwickelt und iterativ verbessert.
  - [README](works/thunderfall/README.md) · [Prompt](works/thunderfall/PROMPT.md) · JavaScript, Canvas 2D, Web Audio.
  - Vorschau: ![THUNDERFALL — Spielaufnahme](assets/screenshots/thunderfall/gameplay.jpg)

- **[IRON BASTION / 钢铁防线](https://iron-bastion.zecoba.workers.dev/)** — Verteidige ein Signalfeuer gegen feindliche Panzerwellen in sechs 3D-Sektoren mit zerstörbaren Ziegelmauern, einem Sprint und einem elektromagnetischen Impuls.
  - Entwickler: [chat01.ai](https://linux.do/u/bandaot)
  - Plattform: Browser mit WebGL 2, chinesische Oberfläche; Tastatur-, Maus- und Touchsteuerung sind im Handbuch beschrieben. Kampf ohne Anmeldung oder Zahlung gestartet.
  - GPT-6 Astra: Nutzung und Aufgaben von GPT-6 Astra müssen noch vom Entwickler bestätigt werden.
  - Vorschau: ![IRON BASTION / 钢铁防线 — Spielaufnahme](assets/screenshots/iron-bastion/gameplay.jpg)

- **[Stick Fighter](https://stick-fighter-production.up.railway.app/)** — Ein Strichmännchen-Kampfspiel in Entwicklung mit Schlägen, Tritten, Aufwärtshaken, Wurfsternen und Paraden; Bot-Training sowie Online- und Freundesmenüs.
  - Entwickler: [Dwayne](https://x.com/CtrlAltDwayne)
  - Plattform: Desktopbrowser, Tastatur; Bot-Training ohne Anmeldung. Online-Mehrspieler und Controller nicht unabhängig getestet.
  - GPT-6 Astra: [X](https://x.com/CtrlAltDwayne/status/2097499157967818780) — Der Entwickler nennt ausdrücklich GPT-6 Astra als Werkzeug für dieses noch unfertige Mehrspieler-Kampfspiel.
  - Vorschau: ![Stick Fighter — Spielaufnahme](assets/screenshots/stick-fighter/gameplay.jpg)

- **[Gogh Strike · Paint Clash](https://gogh-strike.surge.sh/)** — Ein von Van Gogh inspiriertes Farb-FPS mit sechs Künstlern, eigenen Waffen und einem Spielziel von 20 Punkten.
  - Entwickler: [Peter Gostev](https://x.com/petergostev)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/petergostev/status/2096015315209449578) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/gogh-strike/SOURCE.md).
  - Vorschau: ![Gogh Strike · Paint Clash — Spielaufnahme](assets/screenshots/gogh-strike/gameplay.jpg)

- **[ASTEROIDS · Deepfield](https://asteroids-deepfield-cockpit.dan200200.chatgpt.site/)** — Ein Asteroids-Cockpit mit vier Kameras, Radar, Zwillingskanonen und Trägheitsflug.
  - Entwickler: [Eyes Wide Open](https://x.com/DantesClown)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/DantesClown/status/2096085439052452064) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/asteroids-deepfield/SOURCE.md).
  - Vorschau: ![ASTEROIDS · Deepfield — Spielaufnahme](assets/screenshots/asteroids-deepfield/gameplay.jpg)

- **[BLACKWATER · Silent Harbor](https://blackwater-roan.vercel.app/)** — Ein taktischer FPS in einem verregneten Frachtterminal mit detailliertem Gewehr, Kampf-HUD und neun Gegnern.
  - Entwickler: [hiraeth](https://x.com/WoahWurdz)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/WoahWurdz/status/2095958882732355908) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/blackwater/SOURCE.md).
  - Ressourcen: [GitHub](https://github.com/Hiraeth010/blackwater)
  - Vorschau: ![BLACKWATER · Silent Harbor](assets/screenshots/blackwater/gameplay.jpg)

- **[Cinderfall · Fire, Shadow & Steel](https://rogue-omega.vercel.app/)** — Eine Fantasy-Duellarena mit vier Helden, sechs Klassenfähigkeiten, KI-Duellen und Onlineräumen.
  - Entwickler: [JUMPERZ](https://x.com/jumperz)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/jumperz/status/2096600055301984738) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/cinderfall/SOURCE.md).
  - Vorschau: ![Cinderfall · Fire, Shadow & Steel](assets/screenshots/cinderfall/gameplay.jpg)

<a id="puzzles"></a>

### Rätsel und Denkspiele

Logikrätsel, Physikaufgaben, Wortspiele und raffinierte kleine Mechaniken.

- **[瓜体实验室 / Melon Lab](https://melon-game.jack-514.chatgpt.site/)** — Ein Wassermelonen-Fusionsspiel, dessen Spielmechanik auf der Verformung und den Zusammenstößen weicher Früchte beruht.
  - Entwickler: [Ayi1337](https://github.com/Ayi1337)
  - Plattform: Moderner Browser; der Entwickler stellt außerdem eine eigenständige HTML-Version zum Herunterladen bereit.
  - GPT-6 Astra: [One-Shot-Tests und ursprüngliche Prompts des Entwicklers](https://github.com/Ayi1337/gpt6-astra-one-shot-games/blob/main/README.md).
  - Materialien: [Quellcode](https://github.com/Ayi1337/gpt6-astra-one-shot-games/tree/main/melon-lab/src) · [Eigenständige HTML-Datei](https://github.com/Ayi1337/gpt6-astra-one-shot-games/blob/main/melon-lab/%E7%93%9C%E4%BD%93%E5%AE%9E%E9%AA%8C%E5%AE%A4.html)
  - Vorschau: ![Melon-Lab-Spielszene mit weichen Früchten im Verschmelzungsbehälter und Physikreglern.](assets/screenshots/melon-lab/gameplay.jpg)

- **[Q弹消消乐](https://qbxxl.0nz.de/)** — Entferne mindestens drei verbundene Blasen gleicher Farbe; große Gruppen erzeugen Bomben und Regenbögen. Mit Entspannungs-, Level- und Zeitmodus.
  - Entwickler: zding
  - Plattform: Browser, chinesische Oberfläche; Start ohne Anmeldung oder Zahlung geprüft.
  - GPT-6 Astra: Nutzung und Aufgaben von GPT-6 Astra müssen noch vom Entwickler bestätigt werden.
  - Vorschau: ![Q弹消消乐 — Spielaufnahme](assets/screenshots/qbxxl/gameplay.jpg)

- **[榫境 / Sunjing Puzzles](https://sunjing-puzzles.vercel.app)** — Zerlege ein sechsteiliges, ineinandergreifendes Holzpuzzle und löse zwei Huarong-Dao-Schiebepuzzles in einer 3D-Werkstatt, mit Hinweisen und dem Rückgängigmachen von Zügen.
  - Entwickler: [MartinDelophy](https://github.com/MartinDelophy) (Projektinitiator und Einreicher).
  - Plattform: Moderner Browser mit WebGL 2; chinesische Oberfläche sowie Maus-, Tastatur- und Touchsteuerung. Kostenlos, ohne Anmeldung oder API-Schlüssel; der Fortschritt bleibt im aktuellen Browser.
  - Modellbeteiligung: [Entstehungsbericht](works/sunjing-puzzles/CREATION.md) — Iterative Arbeit in Codex an Spieldesign, prozeduraler 3D-Grafik, Regeln, Lösungsalgorithmus und Tests; die genaue Nutzung von GPT-6 Astra muss noch vom Autor bestätigt werden (Entwurfseinreichung).
  - Materialien: [Quellcode und Startanleitung](works/sunjing-puzzles/README.md) · [Anforderungen](works/sunjing-puzzles/PROMPTS.md) · Technik: React, Vinext/Vite, Three.js.
  - Vorschau: ![Sunjings sechsteiliges Holzpuzzle auf einer grünen 3D-Werkbank mit nummerierten Teilen und Steuerelementen zum Herausziehen.](assets/screenshots/sunjing-puzzles/gameplay.jpg)

<a id="strategy-simulation"></a>

### Strategie und Simulation

Tower Defense, strategische Kartenspiele, Aufbau-, Management- und Simulationsspiele mit Sandkastenprinzip.

- **[Tidehook](https://tidehook-mallow.vercel.app/)** — Steuere den kleinen Schlepper Mallow auf drei Küstenfahrten. Das Gewicht der geborgenen Fracht verändert Beschleunigung und Kurvenverhalten; bringe sie zum Hafenkran und berge schließlich eine Leuchtturmlinse.
  - Entwickler: [Emile du Toit / Mafuta Games](https://mafuta.itch.io/)
  - Plattform: Browser auf Desktop-PC oder Laptop, englische Oberfläche; kostenlos, ohne Anmeldung oder Installation. Steuerung per Tastatur oder Klick aufs Wasser; Fortschritt wird im aktuellen Browser gespeichert. Spielen auf einem Smartphone wurde nicht überprüft.
  - GPT-6 Astra: [Angaben des Entwicklers](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/74) — Der Entwickler nutzte GPT-6 Astra in Codex, um das Canvas-Spiel, Schleppphysik, Kranübergabe, Oberfläche, Wetter und Audio schrittweise zu entwickeln und zu verfeinern, und übernahm selbst Gestaltungsvorgaben und Spieltests. Die Grafik entstand separat durch Bildgenerierung; Musik und Klangeffekte werden mit Web Audio synthetisiert.
  - Ressourcen: [itch.io](https://mafuta.itch.io/tidehook) · [Gameplay-Trailer](https://youtu.be/wlEh8gnDOnU) · Natives JavaScript, Canvas 2D, Vite und Web Audio; das Quellcode-Repository ist privat. [Prüfung und Bildquelle (Englisch)](assets/screenshots/tidehook/SOURCE.md)
  - Vorschau: ![Vom Entwickler bereitgestelltes Bild der Veröffentlichung vom 2026-09-11: Mallow schleppt schwere Fracht zwischen den Inseln von The Old Sound; Hafenentfernung und Minikarte sind sichtbar.](assets/screenshots/tidehook/gameplay.png)

- **[Dwellcraft · 住进想象](https://dwellcraft.vercel.app/)** — Richte drei 3D-Häuser ein, passe Materialien und Licht an und begehe sie aus Augenhöhe; mit lokalen Speicherständen und GLB-Import.
  - Entwickler: [Ryan-fm](https://github.com/Ryan-fm)
  - Plattform: Desktopbrowser mit WebGL; chinesische und englische Oberfläche. Kostenlos, ohne Anmeldung. Quest-Hardware ungetestet.
  - Codex: [GitHub](https://github.com/Ryan-fm/Dwellcraft/blob/main/docs/DEVELOPMENT.md) — Iterative Codex-Entwicklung ist dokumentiert; die genaue Zuordnung zu GPT-6 Astra wartet auf Bestätigung des Autors.
  - Ressourcen: [GitHub](https://github.com/Ryan-fm/Dwellcraft)
  - Vorschau: ![Dwellcraft](https://raw.githubusercontent.com/Ryan-fm/Dwellcraft/main/docs/screenshots/editor-en.png)

- **[Little Kingdom Chess / 작은 왕국 체스](https://little-kingdom-chess.echo3042.chatgpt.site/)** — Schach gegen den Computer auf einem drehbaren 3D-Brett mit Miniaturfiguren, Zugverlauf und Rücknahme.
  - Entwickler: [에코_eco](https://x.com/echo3042)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/echo3042/status/2096123409029886250) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/little-kingdom-chess/SOURCE.md).
  - Vorschau: ![Little Kingdom Chess / 작은 왕국 체스 — Spielaufnahme](assets/screenshots/little-kingdom-chess/gameplay.jpg)

- **[JUNK RUN](https://junk-run.pages.dev/)** — Baue aus Schrott ein Fahrzeug ohne Motor und rolle bergab; Start in einer Werkstatt aus der Ich-Perspektive.
  - Entwickler: [TheLabGuy](https://x.com/hermesailab)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/hermesailab/status/2097508053901840850) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/junk-run/SOURCE.md).
  - Vorschau: ![JUNK RUN — Spielaufnahme](assets/screenshots/junk-run/gameplay.jpg)

- **[Spy or Lie](https://spyorlie.com/)** — Ein Strategiespiel auf einem Hexfeld gegen den Computer: verdeckte Agenten platzieren, bluffen und feindliche Gruppen für Kettenreaktionen einkreisen.
  - Entwickler: [Nate Lorenzen](https://x.com/NateLorenzen)
  - Plattform: Browser, englische Oberfläche; erster Zug und Computerantwort ohne Anmeldung oder Zahlung geprüft.
  - GPT-6 Astra: [X](https://x.com/NateLorenzen/status/2097504974683152559) — Laut Entwickler setzte Astra seinen Spielentwurf in einem Durchlauf als spielbare Demo um; der Entwicklungsprozess wurde nicht unabhängig geprüft.
  - Vorschau: ![Spy or Lie — Spielaufnahme](assets/screenshots/spy-or-lie/gameplay.jpg)

- **[三分天下 · 百将风云 / Three Kingdoms: Hundred Heroes](https://sanguo-jiangshan.vercel.app)** — Führe Wei, Shu oder Wu in einem rundenbasierten Feldzug durch 15 Städte: Verwalte Gold und Getreide, befehlige ein Aufgebot von 108 Offizieren mit jeweils eigenem KI-generiertem Porträt und kämpfe gegen KI-Fraktionen um die Einigung des Landes.
  - Entwickler: [MartinDelophy](https://github.com/MartinDelophy) (Projektinitiator und Einreicher).
  - Plattform: Moderner Browser; chinesische Oberfläche sowie Maus- und Touchsteuerung. Kostenlos, ohne Anmeldung oder API-Schlüssel; lokale Spielstände lassen sich importieren und exportieren.
  - GPT-6 Astra: [Entstehungsbericht](works/three-kingdoms/CREATION.md) — Nutzung vom Entwickler bestätigt: iterative Zusammenarbeit in Codex an Regeln, Oberfläche, KI, Spielbalance und Tests. Gelände und Offiziersporträts wurden mit Bildgenerierungswerkzeugen erstellt.
  - Materialien: [Quellcode und Startanleitung](works/three-kingdoms/README.md) · [Anforderungen](works/three-kingdoms/PROMPTS.md) · Technik: React, TypeScript, Vinext/Vite.
  - Vorschau: ![Three-Kingdoms-Spielszene mit einer Städtekarte im Tuschemalstil, Ressourcensteuerung und Offiziersporträts.](assets/screenshots/three-kingdoms/gameplay.jpg)

- **[Last Beacon / 最后的灯塔](https://last-beacon.loupengju.cc)** — Verbinde ein Stromnetz auf einer Miniaturinsel, baue und verbessere Türme und teile die begrenzte Leistung ein, um einen Leuchtturm gegen zehn Wellen und einen Endboss zu verteidigen.
  - Entwickler: [stackloomdev](https://github.com/stackloomdev)
  - Plattform: Moderne Desktop- und Mobilbrowser, mit chinesischer und englischer Oberfläche; kostenlos, ohne Anmeldung oder API-Schlüssel. Optionaler Ton benötigt Web Audio.
  - GPT-6 Astra: [Entwicklungsbericht und Modellbeitrag](https://github.com/stackloomdev/last-beacon/blob/main/docs/CREATION.md) — Für Spieldesign, Code, prozedurale Grafik und Tests in mehreren Iterationen eingesetzt; kein One-Shot-Test.
  - Materialien: [Quellcode und Startanleitung](https://github.com/stackloomdev/last-beacon) · [Anforderungen und Iterationsnotizen](https://github.com/stackloomdev/last-beacon/blob/main/docs/PROMPT.md) · Technik: JavaScript, Canvas 2D, Web Audio.
  - Vorschau: ![Last Beacon auf Englisch: Mit Strom versorgte Geschütztürme, Mörser und ein Frostturm verteidigen die Insel gegen Gegner auf dem Küstenweg.](assets/screenshots/last-beacon/gameplay-en.png)

- **[缺氧 · 小小星球](https://hypoxia-6tu.pages.dev/)** — Überlebenssimulation einer unterirdischen Kolonie: Drei Bewohner graben und bauen, während du Sauerstoff, Nahrung und Strom verwaltest.
  - Entwickler: dudu
  - Plattform: Browser mit Canvas, chinesische Oberfläche, Maus und Tastatur; Start ohne Anmeldung oder Zahlung geprüft.
  - GPT-6 Astra: [Issue #28](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/28) — Laut Einreichung durchgehend mit GPT-6 Astra entwickelt; nicht unabhängig bestätigt.
  - Vorschau: ![缺氧 · 小小星球 — Spielaufnahme](assets/screenshots/hypoxia/gameplay.jpg)

- **[魔塔 · 永夜之阶](https://ai.ywnet.xyz/work/mt.html)** — Ein Pixelabenteuer in einem fünfzehnstöckigen Turm mit Angriffs-/Verteidigungsberechnung, begrenzten Ressourcen und drei Schlüsselfarben.
  - Entwickler: yw
  - Plattform: Browser, chinesische Oberfläche; Start ohne Anmeldung oder Zahlung geprüft.
  - GPT-6 Astra: [Issue #38](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/38) — Die Einreichung enthält den Entwicklungsprompt unter GPT-6 Astra; der genaue Modellbeitrag wurde nicht unabhängig geprüft.
  - Vorschau: ![魔塔 · 永夜之阶 — Spielaufnahme](assets/screenshots/magic-tower/gameplay.jpg)

- **[永恒荒野](https://starve.pages.dev/)** — Ein Survival- und Strategiespiel im Browser, eingereicht als Nachbau von Don't Starve. Der Screenshot zeigt Walderkundung, das Sammeln von Ressourcen, ein Inventar sowie Anzeigen für Gesundheit, Hunger und geistige Verfassung.
  - Entwickler: dudu
  - Plattform: Browser; der Screenshot zeigt eine chinesische Oberfläche und Tastatursteuerung. Zahlungs- und Anmeldebedingungen sind nicht angegeben; die Demo wurde nicht unabhängig überprüft.
  - GPT-6 Astra: [Angabe aus der Einreichung](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/26) — Laut Einreichung wurde die gesamte Entwicklung von GPT-6 Astra durchgeführt.
  - Materialien: Kein öffentlicher Quellcode oder separater Entwicklungsbericht wurde bereitgestellt.
  - Vorschau: ![Screenshot von 永恒荒野 aus Issue #26: eine Figur am Lagerfeuer im Wald, mit Inventar, Überlebensanzeigen und Minikarte.](assets/screenshots/eternal-wilderness/gameplay.jpg)

- **[潜水员戴夫 / Dave the Diver](https://dave-2cm.pages.dev/)** — Eine Browser-Nachbildung von Dave the Diver, die Unterwasserfischen mit der Harpune, den Betrieb eines Sushi-Restaurants und Landwirtschaft auf einer Insel verbindet.
  - Entwickler: dudu
  - Plattform: Browser; chinesische Oberfläche mit Maus- und Tastatursteuerung. Zugriff und Start eines Tauchgangs wurden ohne Anmeldung oder Zahlung überprüft.
  - GPT-6 Astra: [Angabe aus der Einreichung](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/41) — Laut Einreichung wurde die gesamte Entwicklung von GPT-6 Astra durchgeführt.
  - Materialien: Kein öffentlicher Quellcode oder separater Entwicklungsbericht wurde bereitgestellt.
  - Vorschau: ![Spielszene der Dave-the-Diver-Nachbildung: ein Taucher zwischen Fischen mit Anzeigen für Sauerstoff, Tiefe und gefangene Fische.](assets/screenshots/dave-the-diver/gameplay.jpg)

- **[No Moat](https://no-moat.petergyang.chatgpt.site/)** — Ein Startup-Roguelike mit Karten: Rekrutiere ein Team und bekämpfe Nachahmer, Bugs und Cloudrechnungen.
  - Entwickler: [Peter Yang](https://x.com/petergyang)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/petergyang/status/2096297378584375672) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/no-moat/SOURCE.md).
  - Vorschau: ![No Moat](assets/screenshots/no-moat/gameplay.jpg)

- **[The Free Game](https://vale-dos-vinhedos.lucas579686.chatgpt.site/)** — Baue ein detailreiches mittelalterliches 3D-Dorf mit Straßen, Arbeitskräften und Produktionsketten.
  - Entwickler: [Lucas Marques, from Shiva](https://x.com/LucasMarquesSv)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/LucasMarquesSv/status/2096772160404504583) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/the-free-game/SOURCE.md).
  - Ressourcen: [GitHub](https://github.com/LucasMarquesShiva/the-free-game)
  - Vorschau: ![The Free Game](assets/screenshots/the-free-game/gameplay.jpg)

- **[DUST FRONT](https://dust-front.mustafaakin.dev/)** — Ein Einzelspieler-RTS mit Basisbau, Eroberung von Stützpunkten und Boden- und Luftstreitkräften.
  - Entwickler: [Mustafa Akın](https://x.com/mustafaakin)
  - Plattform: Desktop-Browser; Tastatur und Maus, keine Anmeldung erforderlich.
  - GPT-6 Astra: [Prüfnotizen](assets/screenshots/dust-front/SOURCE.md) — Der bereitgestellte Beitrag von Mustafa Akın nennt ChatGPT Astra und Blender MCP sowie etwa 40 Prompts einschließlich der Asset-Erstellung.
  - Vorschau: ![DUST FRONT: Ein Einzelspieler-RTS mit Basisbau, Eroberung von Stützpunkten und Boden- und Luftstreitkräften.](assets/screenshots/dust-front/gameplay.png)

- **[前线指令 / Frontline Command](https://fluffy-biscotti-dad318.netlify.app/)** — Errichte eine Basis, kämpfe um Rohstoffgebiete und befehlige Panzer, Infanterie, Flugzeuge und Drohnen gegen KI-Armeen in einem Echtzeitstrategiespiel mit modernem Kriegsszenario. Spione und Aufklärung verschaffen zusätzliche Vorteile.
  - Ersteller: [嘟啊嘟](https://x.com/HDLhN783wtLkpPR)
  - Plattform: Desktop-Browser, chinesische Oberfläche, Maus und Tastatur; eine Einzelspielerpartie ließ sich ohne Anmeldung, Zahlung oder Installation starten.
  - GPT-6 Astra: [X](https://x.com/HDLhN783wtLkpPR/status/2097321360641122393) — Im verlinkten Beitrag nennt der Ersteller „GPT Astra“ als Werkzeug zur Erstellung dieses Echtzeitstrategiespiels; die genaue Modellversion und der detaillierte Entwicklungsablauf werden nicht angegeben.
  - Verweise: [Einreichung](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/66) · [Prüfnotizen (Englisch)](assets/screenshots/frontline-command/SOURCE.md)
  - Vorschau: ![Frontline Command: Basis, drei ausgewählte Panzer und Platzierung eines Kraftwerks in einer laufenden Partie; v0.8, aufgenommen am 2026-09-09.](assets/screenshots/frontline-command/gameplay.jpg)

<a id="rpg-adventures"></a>

### Rollenspiele und Abenteuer

Rollenspiele, Erkundung, erzählerische Abenteuer und interaktive Geschichten.

- **[The Sunshard](https://mindblown.ai/games/the-sunshard)** — Erkunde ein Action-RPG im Voxel-Stil, bekämpfe Hollowborn mit Spark Bolt und Sunburst, weiche per Teleportation aus und erwecke das Sonnentor.
  - Entwickler: [Mindblown / @mind](https://mindblown.ai/@mind) · [Mindblown](https://mindblown.ai/) · [X](https://x.com/mindblown_ai)
  - Plattform: Desktop-Browser; Tastatur und Maus, keine Anmeldung erforderlich.
  - GPT-6 Astra: Für dieses Spiel unbestätigt: Die vorgelegte Antwort des Erstellers erlaubt die Aufnahme, nennt aber weder Modell noch Rolle. [Prüfnotizen](assets/screenshots/the-sunshard/SOURCE.md).
  - Vorschau: ![The Sunshard — Golden Hollow](assets/screenshots/the-sunshard/gameplay.png)

- **[Lumbridge / Elderwood Realms](https://elderwood-realms.rohannvarma.chatgpt.site/)** — Ein Retro-Mehrspielerabenteuer mit gemeinsamer Welt, Fertigkeiten, Sammeln und Kampf; als Gast spielbar.
  - Entwickler: [Rohan Varma](https://x.com/TheRohanVarma)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/TheRohanVarma/status/2096744577332068549) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/lumbridge/SOURCE.md).
  - Vorschau: ![Lumbridge / Elderwood Realms — Spielaufnahme](assets/screenshots/lumbridge/gameplay.jpg)

- **[Silent Meridian / 静默子午线](https://silent-meridian.stackloom.org/)** — Ein atmosphärisches Point-and-Click-Abenteuer mit vier Kapiteln und dreizehn Rätseln: Hinweise aus Gegenwart und Echo vergleichen, Beweise sammeln und das Geheimnis einer um 00:17 erstarrten Sternwarte aufdecken.
  - Ersteller: [stackloomdev](https://github.com/stackloomdev)
  - Plattform: Desktop- und Mobilbrowser; Chinesisch und Englisch. Kostenlos direkt online spielbar, ohne Anmeldung, Installation oder API-Schlüssel. Optionale WebGL-Effekte; bei fehlender Unterstützung bleiben die Originalillustrationen sichtbar.
  - GPT-6 Astra: [Entwicklungsbericht](https://github.com/stackloomdev/silent-meridian/blob/main/docs/CREATION.md) — Iterative Zusammenarbeit in Codex an Geschichte, Rätseln, zweisprachigen Texten, Code, prozeduralen 3D-Effekten und Tests.
  - Ressourcen: [Quellcode und Startanleitung](https://github.com/stackloomdev/silent-meridian) · [Herkunft der Grafiken](https://github.com/stackloomdev/silent-meridian/blob/main/docs/ART.md) · Technik: JavaScript, WebGL, Web Audio.
  - Vorschau: ![Silent Meridian im Spiel: Sternwarte mit einem 3D-Messingmechanismus, Hinweismarkierungen, Gegenwart/Echo-Umschaltung und Ermittlungsjournal.](assets/screenshots/silent-meridian/gameplay.png)

- **[热血归来 · 八荒幻世 / Mir176 Dragon Warrior](https://mir176-dragon-warrior.geekcatxx.chatgpt.site/)** — Ein von Legend inspiriertes Action-RPG mit Krieger, Magier und Taoist, Ausrüstung, Dungeons und automatischem Kampf.
  - Entwickler: [知识猫AI实验室](https://x.com/GeekCatX)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/GeekCatX/status/2097530887558865115) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/mir176/SOURCE.md).
  - Vorschau: ![热血归来 · 八荒幻世 / Mir176 Dragon Warrior](assets/screenshots/mir176/gameplay.jpg)

- **[Zork · The Great Underground Empire](https://zork-underground-empire.netlify.app/)** — Eine inoffizielle 3D-Adaption von Zork mit Erkundung aus der Ego-Perspektive, Rätseln, Kämpfen und Tagebuch.
  - Entwickler: [Ethan Mollick](https://x.com/emollick)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/emollick/status/2096047660662722620) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/zork/SOURCE.md).
  - Vorschau: ![Zork · The Great Underground Empire](assets/screenshots/zork/gameplay.jpg)

<a id="platformers-racing"></a>

### Plattform- und Rennspiele

Parkour, Plattformherausforderungen, Rennen und Spiele rund um Bewegung und Wege.

- **[Barrelbound: The Lost Cargo](https://barrelbound.vercel.app/)** — Wähle Rocco oder Pip für drei Dschungel-Parcours: Nutze Doppelsprünge, wirf Fässer, fahre mit einer Lore und sammle die verlorene Fracht auf dem Weg zum Endboss ein.
  - Entwickler: [Emile du Toit / Mafuta Games](https://mafuta.itch.io/)
  - Plattform: Browser auf Desktop-PC oder Laptop, englische Oberfläche; kostenlos, ohne Anmeldung oder Installation. Tastatursteuerung; Controller-Unterstützung laut Entwickler.
  - GPT-6 Astra: [Angaben des Entwicklers](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/73) — Der Entwickler nutzte GPT-6 Astra in Codex für Implementierung, Spielmechaniken, Überarbeitung und Tests und übernahm selbst die Gestaltungsvorgaben und Spieltests. Die Grafik entstand separat durch Bildgenerierung; Musik und Klangeffekte werden mit Web Audio synthetisiert.
  - Ressourcen: [itch.io](https://mafuta.itch.io/barrelbound) · [Gameplay-Trailer](https://youtu.be/X87UqF_n3ro) · Phaser 3, Vite und Web Audio; das Quellcode-Repository ist privat. [Prüfung und Bildquelle (Englisch)](assets/screenshots/barrelbound/SOURCE.md)
  - Vorschau: ![Vom Entwickler bereitgestelltes Bild der Veröffentlichung vom September 2026: Rocco, Fässer und Bananenpfade auf den Holzplattformen von Jungle Dock Dash.](assets/screenshots/barrelbound/gameplay.jpg)

- **[STORM RACE](https://storm-race.vercel.app/)** — Mini-4WD-Rennen mit Explosionsansicht der Teile, Boost sowie trockenen, regnerischen und stürmischen Streckenbedingungen.
  - Entwickler: [BubuAi](https://x.com/BubuStd)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/BubuStd/status/2096587056755638553) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/storm-race/SOURCE.md).
  - Vorschau: ![STORM RACE — Spielaufnahme](assets/screenshots/storm-race/gameplay.jpg)

- **[FANG STARLIGHT RUN](https://fang-starlight-run.yosshy666.chatgpt.site/)** — Ein Wolf sammelt Münzen und Sternfragmente in drei Mondlicht-Leveln mit Doppelsprung und Dash.
  - Entwickler: [FANGプラス最強伝説🐺](https://x.com/FANGsaikyou)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/FANGsaikyou/status/2096192022596194588) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/fang-starlight-run/SOURCE.md).
  - Vorschau: ![FANG STARLIGHT RUN — Spielaufnahme](assets/screenshots/fang-starlight-run/gameplay.jpg)

- **[Blue Bajaj Rally](https://bajaj.guzo.tech/)** — Fahre einen dreirädrigen Bajaj auf einer äthiopisch inspirierten Hochlandstrecke gegen fünf KI-Fahrer oder die Uhr.
  - Entwickler: [Guzo Technologies](https://x.com/guzotech)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/guzotech/status/2096209787864088638) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/blue-bajaj-rally/SOURCE.md).
  - Vorschau: ![Blue Bajaj Rally — Spielaufnahme](assets/screenshots/blue-bajaj-rally/gameplay.jpg)

- **[APEX CLUB — Bay Kart Grand Prix](https://apex-club-racing.vercel.app)** — Fahre drei Runden auf Bay Circuit, wähle aus sechs Karts und lade Mini-Turbos für den Kurvenausgang auf, um in der Einzelwertung aufzusteigen oder Punkte für ein 4-gegen-4-Team zu sammeln.
  - Entwickler: Ryan
  - Plattform: Desktop-Browser mit WebGL 2 und Tastatur; kostenlos, ohne Anmeldung oder API-Schlüssel. Läuft auf einem lokalen HTTP-Server oder statischem Hosting; Three.js ist lokal enthalten. Teamrennen finden lokal mit einem Menschen und sieben KI-Fahrern statt.
  - Modellbeteiligung: [Entwicklungsbericht](works/apex-club/CREATION.md) — Iterative Arbeit in Codex an Spielmechanik, Code, prozeduraler Grafik und Tests; die Zuordnung zu GPT-6 Astra muss noch vom Entwickler bestätigt werden.
  - Materialien: [Quellcode und Startanleitung](works/apex-club/README.md) · [Anforderungen und Iterationsnotizen](works/apex-club/PROMPTS.md) · [Geteiltes GPT-Gespräch](https://chatgpt.com/s/cx_6a9e84c13c9c8191bcb7ad0801288adc) · Technik: JavaScript, Three.js.
  - Vorschau: ![APEX CLUB-Teamrennen auf Bay Circuit mit nahen Karts, Rundenfortschritt, aktuellen Teampunkten und Minikarte.](assets/screenshots/apex-club/gameplay.png)

- **[PELICAN PEDAL / 鹈鹕踏浪](https://pelican-pedal.zecoba.workers.dev/)** — Fahre mit einem Pelikan auf dem Fahrrad an einer wechselnden 3D-Küste entlang: Wechsle zwischen drei Spuren, springe oder ducke dich vor Hindernissen, sammle Fischkombos und nutze Schilde, Magnete und einen sechs Sekunden langen Sprint mit Unverwundbarkeit.
  - Entwickler: [chat01.ai](https://chat01.ai) (laut Einreichung).
  - Plattform: Desktop- und Mobilbrowser mit Tastatur- oder Touchsteuerung; laut Einreichung kostenlos und ohne Anmeldung. Die Demo wurde nicht unabhängig überprüft.
  - GPT-6 Astra: [Issue #10](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/10) — Laut Einreichung mit einem einzigen Prompt erstellt; der verlinkte Entwicklungsbericht wurde nicht unabhängig überprüft.
  - Materialien: [Geteilter Entwicklungsbericht](https://chat01.ai/zh/chat/01M1VJ5K55803V2YK3T2NKTX33)
  - Vorschau: ![Titelbildschirm von PELICAN PEDAL mit einem radelnden Pelikan am Meer; Screenshot aus Issue #10.](https://github.com/user-attachments/assets/ec085afa-3c45-4de6-b03f-5c626f941546)

- **[狂飙赛车 · OVERDRIVE](https://gpt-kbsc.0nz.de/)** — 3D-Rennen gegen fünf KI-Gegner, mit Fahrzeug- und Streckenauswahl, Zeitfahren, Drifts und Nitro.
  - Entwickler: zding
  - Plattform: Browser, chinesische Oberfläche; Start ohne Anmeldung oder Zahlung geprüft.
  - GPT-6 Astra: Nutzung und Aufgaben von GPT-6 Astra müssen noch vom Entwickler bestätigt werden.
  - Vorschau: ![狂飙赛车 · OVERDRIVE — Spielaufnahme](assets/screenshots/overdrive/gameplay.jpg)

- **[零界深潜 / ABYSS PROTOCOL](https://abyss-descent.zecoba.workers.dev/)** — Bewege dich nach links und rechts durch einen 3D-Schacht mit beweglichen, brüchigen und phasenweise verschwindenden Plattformen, weiche Lasern und Sägen aus und sammle Kristalle und Überlebenschips.
  - Entwickler: [chat01.ai](https://linux.do/u/bandaot)
  - Plattform: Desktop-Browser, chinesische Oberfläche, A/D oder Pfeiltasten. Start ohne Anmeldung oder Zahlung geprüft; Mobilgeräte nicht getestet.
  - Codex: [Chat01](https://chat01.ai/en/chat/01M221KFTPTBQV5ARQWJM7P86Y) — Das geteilte Protokoll nennt GPT-6 Pro und dokumentiert die Spielerstellung und iterative Überarbeitungen; GPT-6 Astra wird nicht ausdrücklich genannt.
  - Ressourcen: [HTML](https://files.chat01.ai/python-generations/9ab8f4f7-425f-4709-b741-7218d87122e6/abyss-descent.html) · [ZIP](https://files.chat01.ai/python-generations/9ab8f4f7-425f-4709-b741-7218d87122e6/abyss-descent-complete.zip)
  - Vorschau: ![零界深潜 / ABYSS PROTOCOL](assets/screenshots/abyss-protocol/gameplay.jpg)

- **[疾风赛道 / Kart Racing（跑跑卡丁车）](https://qwen3-8-test.vercel.app/)** — Drei Runden mit Drifts, Nitro und Gegenständen; das Spiel heißt aktuell 疾风赛道 und bietet einen Onlinemodus für 2–4 Spieler.
  - Entwickler: [SteveZhang / jay6697117](https://github.com/jay6697117) · [X](https://x.com/stevezh10058543)
  - Plattform: Browser, kostenlos, ohne Anmeldung. Laut Entwickler kann VPN/Proxy nötig sein. Solostart geprüft; Mehrspieler nicht getestet.
  - GPT-6 Astra: [Issue #51](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/51) — Laut Entwickler: Erstversion mit Qwen3.8 Max, zweite Version mit Astra vollständig neu aufgebaut.
  - Vorschau: ![疾风赛道 / Kart Racing（跑跑卡丁车）](https://github.com/user-attachments/assets/015e0ca1-7032-4d0e-9391-ad3f40d84227)

- **[混合马里奥Ⅱ · 忍者龙剑传 × 坦克大战 / Mario Mix II](https://aha-xiaoq.github.io/games/mario-mix-2/play.html)** — Erkunde Marios unterirdische Welt 1-2 mit Ryu Hayabusa aus Ninja Gaiden und dem Panzer aus Battle City: Springen, Wandklettern und Kämpfen in Seitenansicht mit Ryu, Kämpfe aus der Vogelperspektive mit dem Panzer oder eine Ninja-Panzer-Staffel zur Rettung der Prinzessin.
  - Ersteller: [在下_小Q（Aha-xiaoQ）](https://github.com/Aha-xiaoQ)
  - Plattform: Desktop-Browser, chinesische Oberfläche, Tastatur empfohlen; kostenlos, ohne Anmeldung oder Installation.
  - GPT-6 Astra: [Issue #65](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/65) — Laut Ersteller wurde GPT-6 Astra zur schrittweisen Entwicklung und Fehlerbehebung von Spielcode und Interaktionen genutzt, mit eigenen Spielideen und Rückmeldungen des Erstellers; keine Generierung mit nur einem Prompt.
  - Ressourcen: [Projektvorstellung](https://aha-xiaoq.github.io/games/mario-mix-2/) · [Video](https://www.bilibili.com/video/BV1erYt69EFP/) · [Prüfung und Bildquellen (Englisch)](assets/screenshots/mario-mix-2/SOURCE.md)
  - Rechte: Inoffizielles Fanspiel; die Rechte an klassischen Figuren, Bildern und Musik verbleiben bei den jeweiligen Rechteinhabern. Materialnachweise stehen auf der ursprünglichen Spielseite.
  - Vorschau: ![Mario Mix II — vom Ersteller bereitgestelltes Videotitelbild, keine Spielaufnahme.](https://aha-xiaoq.github.io/games/mario-mix-2/cover.jpg)
  - Screenshot: ![Der Panzer in Mario Mix II feuert am Eingang zu Welt 1-2; laufende Version 1.0, aufgenommen am 2026-09-09.](assets/screenshots/mario-mix-2/gameplay.jpg)

<a id="experimental-multiplayer"></a>

### Experimentelles und Mehrspieler

Ungewöhnliche Spielmechaniken, Online-Wettkämpfe und kooperative Erlebnisse.

- **[ASTRA Arcade](https://astra-arcade.antonioleivag.chatgpt.site/)** — Sechs Browserspiele mit Bergrallye, Snowboard und Antigravitationsrennen; als eine Sammlung gezählt.
  - Entwickler: [Antonio Leiva](https://x.com/antonioleivag)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/antonioleivag/status/2096509898481651770) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/astra-arcade/SOURCE.md).
  - Vorschau: ![ASTRA Arcade — Spielaufnahme](assets/screenshots/astra-arcade/gameplay.jpg)

- **[Chao Party](https://chao.party/)** — Ein inoffizielles Chao-Garden-Fanspiel für mehrere Spieler: Sonic-Figur wählen und mit Chao im Garten interagieren.
  - Entwickler: [Hank](https://x.com/h4nkdog)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/h4nkdog/status/2097308970431987857) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/chao-party/SOURCE.md).
  - Vorschau: ![Chao Party — Spielaufnahme](assets/screenshots/chao-party/gameplay.jpg)

- **[ORBITAL GARDEN · 轨道花园](https://orbital-garden-one.vercel.app)** — Ein interaktiver Sandkasten für Partikelkunst: 48.000 Lichtpunkte in eine Blume, einen Gravitationsring oder eine Galaxie verwandeln, die Skulptur beeinflussen und einen Moment als Poster exportieren.
  - Entwickler: [jackroc](https://github.com/jackroc)
  - Plattform: Moderner Browser mit WebGL; ein Klick auf den Titel startet das Online-Erlebnis. Kostenlos, ohne Anmeldung oder API-Schlüssel; die eigenständige HTML-Datei funktioniert auch offline. Optionaler Umgebungston benötigt Web Audio.
  - GPT-6 Astra: [Entstehungsbericht und Modellbeitrag](works/orbital-garden/README.md#模型与创作记录) — Der Entwickler nutzte GPT-6 Astra ultra für Konzept, Code und Texte mit gemeinsamer Prüfung; kein One-Shot-Test.
  - Materialien: [Quellcode und Startanleitung](works/orbital-garden/README.md) · [Eigenständige HTML-Datei](works/orbital-garden/index.html) · [Prompt](works/orbital-garden/PROMPT.md) · Technik: natives WebGL, Web Audio, Canvas 2D.
  - Vorschau: ![Mintgrüne Partikelblume in Orbital Garden mit Formauswahl, Reglern für Lebenskraft und Zeitgeschwindigkeit sowie Posterexport.](assets/screenshots/orbital-garden/gameplay.jpg)

- **[Toy2Game / 在线玩具箱](https://games.asmo.top/)** — Vier 3D-Umsetzungen von Tischspielzeug: Schlage Eis unter einem Pinguin heraus, führe Kaninchen durch Fallen, balanciere Astronauten auf einer Plattform und löse Ausparkpuzzles.
  - Entwickler: [asmoyou](https://github.com/asmoyou)
  - Plattform: Browser auf Computern, Tablets und Smartphones; kostenlos, ohne Anmeldung. Lokal am selben Bildschirm für 2–4 abwechselnd spielende Teilnehmer einschließlich Computergegnern, dazu Einzelspieler-Puzzles; kein Online-Mehrspieler. Demo nicht unabhängig überprüft.
  - GPT-6 Astra: [Angabe des Autors](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/24) — Laut Autor hat GPT-6 Astra die Spiele anhand von Spielzeugbeschreibungen und Funktionsanforderungen selbstständig implementiert und getestet.
  - Materialien: [Quellcode und Startanleitung](https://github.com/asmoyou/toy2game) · [Nichtkommerzielle Lizenz](https://github.com/asmoyou/toy2game/blob/main/LICENSE) — Quellcode einsehbar; nichtkommerzielle Nutzung kostenlos, kommerzielle Nutzung nur mit vorheriger schriftlicher Genehmigung des Autors. Technik: TypeScript, React, Vite, Three.js.
  - Vorschau: ![Toy2Game-Übersicht mit vier tatsächlichen 3D-Spielszenen: Pinguineis, Kaninchenfallen, Astronautenbalance und Ausparkpuzzle. Bild von asmoyou.](https://raw.githubusercontent.com/asmoyou/toy2game/main/docs/images/toy2game-overview.png)

- **[Jelly Baby](https://jelly.scottsun.io/)** — Ein Physikspielplatz auf einem sonnigen Holztisch: Gelee hüpfen und dehnen lassen, mit Schaukel und Trampolin.
  - Entwickler: [Scott](https://x.com/scottstts)
  - Plattform: Desktopbrowser; ohne Anmeldung oder Zahlung geöffnet. Mobil nicht getestet.
  - GPT-6 Astra: [X](https://x.com/scottstts/status/2096364764054131119) — Der Entwickler nennt Astra als Werkzeug für die Entwicklung dieses Projekts. [Prüfnotizen (Englisch)](assets/screenshots/jelly-baby/SOURCE.md).
  - Ressourcen: [GitHub](https://github.com/scottstts/Jelly-Baby)
  - Vorschau: ![Jelly Baby](assets/screenshots/jelly-baby/gameplay.jpg)

- **[泡泡坦克大作战联机版 / Toon Tank Arena](https://toon-tank-arena.jay6697117.deno.net/)** — Verteidige einen Regenbogenkern mit abprallenden Geschossen und Extras; mit Solo-, lokalem Koop- und Online-Kampfmodus.
  - Entwickler: [SteveZhang / jay6697117](https://github.com/jay6697117) · [X](https://x.com/stevezh10058543)
  - Plattform: Browser, kostenlos, ohne Anmeldung. Laut Entwickler kann VPN/Proxy nötig sein. Solostart geprüft; Mehrspieler nicht getestet.
  - GPT-6 Astra: [Issue #52](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/52) — Laut Entwickler: Erstversion mit GPT-6 Astra Pro, spätere Verbesserungen mit GPT-6 Astra in Codex.
  - Vorschau: ![泡泡坦克大作战联机版 / Toon Tank Arena](https://github.com/user-attachments/assets/713d44f3-a77c-452c-ba6d-1231882dc670)

## Was ein Eintrag enthält

Eine hilfreiche Empfehlung erklärt, worum es im Spiel geht, wo man es ausprobieren kann und warum es in diese Liste gehört.

| Angabe | Inhalt |
| --- | --- |
| Spiel und Urheber | Spieltitel und Link zum ursprünglichen Entwickler oder Team |
| Das Besondere am Spiel | Ein Satz zur zentralen Spielmechanik, ohne leere Werbeversprechen |
| Zugang | Ein direkt spielbarer Browser-Link ohne Download, Installation, lokale Einrichtung oder Anmeldung |
| Screenshot aus dem Spiel | Mindestens ein echter Screenshot mit einer öffentlich zugänglichen Bild-URL |
| Plattform und Voraussetzungen | Browser / Desktop / Mobilgerät; Kosten, Anmeldung oder besondere Hardwareanforderungen angeben |
| Beitrag von Astra | Eine Erklärung des Entwicklers, ein Entwicklungsbericht oder ein öffentlicher Nachweis zum Beitrag von GPT-6 Astra |
| Entwicklungsmaterialien | Quellcode, Technik und Entstehungsgeschichte, soweit verfügbar |

Angaben zur Modellnutzung beruhen auf öffentlichen Aussagen des Entwicklers; unbelegte Behauptungen werden nicht als bestätigte Tatsachen dargestellt. Spielbare Prototypen sind willkommen, und Open Source ist keine Voraussetzung.

## Die Liste gemeinsam pflegen

[Schlage ein Spiel über ein Issue vor](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/new?template=submit-game.yml) oder eröffne eine Pull Request. Das Eintragsformat steht im [Beitragsleitfaden](CONTRIBUTING.md).

Bitte [melde defekte Links oder falsche Angaben](https://github.com/MartinDelophy/awesome-gpt-6-astra/issues/new?template=broken-link.yml), auch fehlerhafte Urheberangaben oder geänderte Zugangsbedingungen.

## Lizenz

Die eigens für dieses Repository verfassten Listentexte und erstellten Grafiken werden unter [CC0 1.0](LICENSE) der Allgemeinheit gewidmet. Verlinkte Spiele, Quellcodes, Bilder, Marken und andere Inhalte Dritter behalten ihre jeweiligen Lizenzen und Rechte. Ein Eintrag verändert ihre Nutzungsrechte nicht.
