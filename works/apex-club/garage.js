...
const camera=new THREE.PerspectiveCamera(37,1,.1,200);scene.add(new THREE.HemisphereLight(0xe0faff,0x4b5459,2));
for(const [color,intensity,position] of [[0xffe4c5,3.5,[20,35,25]],[0x68d7eb,3,[-22,15,-20]],[0xffffff,2,[-15,18,25]]]){const light=new THREE.DirectionalLight(color,intensity);light.position.set(...position);scene.add(light);if(color===0xffe4c5){light.castShadow=true;light.shadow.mapSize.set(2048,2048);Object.assign(light.shadow.camera,{left:-20,right:20,top:20,bottom:-20,near:1,far:90});light.shadow.normalBias=.025;}}
const envScene=new THREE.Scene();envScene.background=new THREE.Color(0x758b92);
for(const [x,y,z,w,h] of [[-15,10,8,7,20],[15,12,-10,8,22],[0,25,0,20,10]]){const m=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({color:0xffffff}));m.position.set(x,y,z);m.lookAt(0,2,0);envScene.add(m);}const pmrem=new THREE.PMREMGenerator(renderer),environment=pmrem.fromScene(envScene,.02);scene.environment=environment.texture;pmrem.dispose();envScene.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});
...
document.querySelectorAll('[data-color]').forEach(b=>b.addEventListener('click',()=>{car.traverse(o=>{if(o.material?.isMeshPhysicalMaterial&&o.material.clearcoat===1&&o.material.roughness===.27)o.material.color.setHex(parseInt(b.dataset.color,16));});document.querySelectorAll('[data-color]').forEach(x=>x.classList.toggle('active',x===b));}));
